...
| ⚡ 推理服务 | **vLLM**（主，含 **vLLM-MLX** Apple Silicon）· SGLang（对比，含 **SGLang-MLX**）· Transformers（本地） | OpenAI API 兼容（流式 SSE）；Paged Attention / Continuous Batching / KV Cache 全旋钮化；多引擎适配器 |
...
minillm serve --config configs/serve/vllm_mac.yaml        # vLLM-MLX（Apple Silicon，见 docs/serving.md）
SGLANG_USE_MLX=1 minillm serve --config configs/serve/sglang_mac.yaml   # SGLang-MLX（Apple Silicon）
